...
async function main() {
  ...
  `)
    ...
    const nodes = [
        {
            ...
            workerId: "0xe028af412138fe0a31ab0b3671243bdbe19d1a164837b04e7d8d355091fcd844",
        },
        {
            pruntimeURL: "https://phat-cluster-de.phala.network/pruntime-03",
            workerId: "b063d754602f22a3ac2af01ebdb2140357e3ca3d102e55a4d44a751fcb03b040",
        },
        {
            pruntimeURL: "https://phat-cluster-de.phala.network/pruntime-04",
            workerId: "6628b623e2a9b795b57f8dc91c5718b7b63722e1ee617030845a967ff0c8c72e",
        },
        {
            pruntimeURL: "https://phat-cluster-de.phala.network/pruntime-05",
            workerId: "9099308d294e320e001d567b21cee3177d149da08a2f3e7534e7f369d93f4e5e",
        },
    ]
    // pick random one
    ...
    const brickProfileContractId = process.env.PHAT_BRICKS_MAINNET_CONTRACT_ID
    if (!brickProfileContractId) {
        throw new Error("Please set PHAT_BRICKS_MAINNET_CONTRACT_ID via .env file first.")
    }
    ...
    const rollupAbi = new Abi(fs.readFileSync("./abis/action_offchain_rollup_mainnet.json", "utf8"))
    const codeHash = rollupAbi.info.source.wasmHash.toHex()

    const {output} = await registry.systemContract!.query["system::codeExists"]<Bool>(
        pair.address,
        {cert},
        codeHash,
        "Ink"
    ...
    // estimate gas fee & storage deposit fee, check cluster balance enough or not.
    {
        ...
        const storageDepositeFee = !estimate.storageDeposit.isCharge
            ? 0
            : ((registry.clusterInfo?.depositPerByte?.toNumber() ?? 0) * ((source.length / 2) * 2.2)) / 1e12
        const minRequired = gasFee + storageDepositeFee

        const onchainBalance =
            (await apiPromise.query.system.account<Account>(pair.address)).data.free.toNumber() / 1e12
    ...
    const name = `Phat OTP ${num}`

    // estimate gas fee & storage deposit fee, check cluster balance enough or not.
    {
        const estimate = await brickProfile.query.addWorkflow(cert.address, {cert}, name, actions)
    ...
    await signAndSend(brickProfile.tx.addWorkflow({gasLimit}, name, actions), pair)
    ...
    while (true) {
        ...
        if (blocknum > initBlockNum + confirmations) {
            throw new Error(`Wait for transaction finalized in PRuntime but timeout after ${confirmations} blocks.`)
        }
        const {output: numberQuery} = await brickProfile.query.workflowCount<u16>(pair.address, {cert})
    ...
    await signAndSend(brickProfile.tx.authorizeWorkflow({gasLimit: 1000000000000}, num, externalAccountId), pair)
